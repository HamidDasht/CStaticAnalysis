           ...
           token_buffer *tokenbuffer)
{
  ...
  int c;
  size_t i, n;
  word isdelim[((127 * 2 + 1) + bits_per_word) / bits_per_word];
  ...
  int iii = 0;
  for (__CrestChar(&c); c >= 0 && get_nth_bit (c, isdelim); __CrestChar(&c))
  {
    iii++;
    if (iii > 2)
      c = 0;
