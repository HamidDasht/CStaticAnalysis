# 1 "regex.c"
# 1 "/home/ubuntu/Desktop/Chameleon/benchmarks/grep-2.2/src//"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 3300 "regex.c"
          if (p + j < pend)
            {
              if (!(((fail_stack.avail == fail_stack.size) && !((fail_stack).size > (unsigned) (re_max_failures * (5 * 3 + 4)) ? 0 : ((fail_stack).stack = (fail_stack_elt_t *) (destination = (char *) 
# 3302 "regex.c" 3 4
                  __builtin_alloca (
# 3302 "regex.c"
...
# 4381 "regex.c"
    ...
    p1++;
                  do { do { (mcnt) = *(p1) & 0377; (mcnt) += ((signed char) (*((p1) + 1))) << 8; } while (0); (p1) += 2; } while (0);
                  do { char *destination; s_reg_t this_reg; ; ; ; ; ; ; ; while (((fail_stack).size - (fail_stack).avail) < (((0 ? 0 : highest_active_reg - lowest_active_reg + 1) * 3) + 4)) { if (!((fail_stack).size > (unsigned) (re_max_failures * (5 * 3 + 4)) ? 0 : ((fail_stack).stack = (fail_stack_elt_t *) (destination = (char *) 
# 4400 "regex.c" 3 4
                 __builtin_alloca (
# 4400 "regex.c"
                 ((fail_stack).size << 1) * sizeof (fail_stack_elt_t)
