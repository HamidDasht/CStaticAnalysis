...
# 184 "array.c"
 if (len > 0) {
  ...
  case 0:
   do {
    htmp = (h << 6); h = *s++ + htmp + (htmp << 10) - h;
  case 7: htmp = (h << 6); h = *s++ + htmp + (htmp << 10) - h;
  case 6: htmp = (h << 6); h = *s++ + htmp + (htmp << 10) - h;
  case 5: htmp = (h << 6); h = *s++ + htmp + (htmp << 10) - h;
  case 4: htmp = (h << 6); h = *s++ + htmp + (htmp << 10) - h;
  case 3: htmp = (h << 6); h = *s++ + htmp + (htmp << 10) - h;
  case 2: htmp = (h << 6); h = *s++ + htmp + (htmp << 10) - h;
  case 1: htmp = (h << 6); h = *s++ + htmp + (htmp << 10) - h;
   } while (--loop);
  }
 }
...
NODE *symbol, *subs;
{
 ...
 if (symbol->type == Node_param_list)
  symbol = stack_ptr[symbol->sub.nodep.number];
 if ((symbol->flags & 512) != 0)
  set_loc("array.c", 238), r_fatal("attempt to use scalar as array");
...
NODE *symbol, *subs;
{
 ...
 (void) ({NODE *_ts = (subs); ((_ts->flags & 16) && (_ts->sub.val.idx == -1 || _ts->sub.val.idx == CONVFMTidx)) ? _ts : r_force_string(_ts);});
 ...
 if (do_lint && subs->sub.val.slen == 0)
  warning("subscript of array `%s' is null string",
   symbol->vname);


 symbol->sub.nodep.x.xl++;
 if ((symbol->flags & 256) == 0
     && symbol->sub.nodep.x.xl/symbol->sub.nodep.l.ll > 10) {
  grow_table(symbol);
 ...
 if (nextfree) bucket = nextfree, nextfree = nextfree->sub.nodep.l.lptr; else bucket = more_nodes();
 bucket->type = Node_ahash;
 if (subs->flags & 2)
  bucket->sub.ahash.name = dupnode(subs);
 else {
 ...
 bucket->sub.ahash.name->flags &= ~64;
 bucket->sub.ahash.name->flags |= 8;
 bucket->sub.ahash.value = Nnull_string;
 bucket->sub.ahash.next = symbol->sub.nodep.r.av[hash1];
 symbol->sub.nodep.r.av[hash1] = bucket;
 return &(bucket->sub.ahash.value);
}
...
NODE *symbol, *tree;
{
 ...
 ((bucket)->flags &= ~512, (bucket)->sub.nodep.l.lptr = nextfree, nextfree = (bucket));
 symbol->sub.nodep.x.xl--;
 if (symbol->sub.nodep.x.xl <= 0) {
  memset(symbol->sub.nodep.r.av, '\0',
   sizeof(NODE *) * symbol->sub.nodep.l.ll);
...
# 467 "array.c"
 ...
 newsize = oldsize = symbol->sub.nodep.l.ll;
 ...
 for (i = 0; i < oldsize; i++) {
  ...
  for (chain = old[i]; chain != ((void *)0); chain = next) {
   ...
   chain->sub.ahash.next = new[hash1];
   new[hash1] = chain;
 ...
 symbol->sub.nodep.r.av = new;
 symbol->sub.nodep.l.ll = newsize;
}
