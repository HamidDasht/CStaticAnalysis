...
# 169 "node.c"
  ...
  s->sub.val.idx = (char) index;

 } else {


  register long num = (long) val;
  if (num < (sizeof(values)/sizeof(values[0])) && num >= 0) {
   sp = (char *) values[num];
   s->sub.val.slen = 1;
  } else {
...
# 291 "node.c"
 ...
 if ((flags & 1) != 0) {
  ...
  end = &(r->sub.val.sp[len]);
  for (pf = ptm = r->sub.val.sp; pf < end;) {
   c = *pf++;
   if (c == '\\') {
    c = parse_escape(&pf);
    if (c < 0) {
...
size_t len;
{
 ...
 r = make_str_node((s), (size_t) (len), 0);
 r->flags |= 2;
 return r;
}
...
# 391 "node.c"
 ...
 if ((tmp->flags & (1|2)) != 0) {
  ...
  ((tmp)->flags &= ~512, (tmp)->sub.nodep.l.lptr = nextfree, nextfree = (tmp));
  return;
 }
 if ((tmp->flags & 2048) != 0) {
  ((tmp)->flags &= ~512, (tmp)->sub.nodep.l.lptr = nextfree, nextfree = (tmp));
