...
extern void gawk_srandom (unsigned int seed);

extern NODE **fields_arr;
extern int output_is_tty;

static NODE *sub_common (NODE *tree, int how_many, int backdigs) __attribute__((crest_skip));
NODE *format_tree (const char *, int, NODE *);
# 79 "builtin.c"
...
# 218 "builtin.c"
 ...
 if (IGNORECASE) {
  ...
  while (l1 > 0) {
   if (l2 > l1)
    break;
   if (*p1 == *p2
       && (l2 == 1 || ((l2) && *(p1)== *(p2) && strncmp((p1), (p2), (size_t) (l2)) == 0))) {
    ret = 1 + s1->sub.val.slen - l1;
    break;
   }
   l1--;
 ...
 do { if ((s1)->flags&2) { unref(s1); }} while (0);
 do { if ((s2)->flags&2) { unref(s2); }} while (0);
 return mk_number(((double) ret), (unsigned int)(1|2|32|64));
}
...
# 510 "builtin.c"
  ...
  case 'l':
   if (big)
    ...
    if (do_posix)
     set_loc("builtin.c", 552), r_fatal("'l' is not permitted in POSIX awk formats");
   }
   big = 1;
   goto retry;
  case 'L':
   if (bigbig)
    ...
    if (do_posix)
     set_loc("builtin.c", 567), r_fatal("'L' is not permitted in POSIX awk formats");
   }
   bigbig = 1;
   goto retry;
  case 'h':
   if (small)
    ...
    if (do_posix)
     set_loc("builtin.c", 582), r_fatal("'h' is not permitted in POSIX awk formats");
   }
   small = 1;
   goto retry;
  case 'c':
...
# 588 "builtin.c"
   ...
   if (arg->flags & 64) {
    ...
    uval = (unsigned long) arg->sub.val.fltnum;
...
# 772 "builtin.c"
  ...
  if (toofew)
   set_loc("builtin.c", 781), r_fatal("%s\n\t`%s'\n\t%*s%s",
...
# 859 "builtin.c"
 ...
 return mk_number(((double) sqrt(arg)), (unsigned int)(1|2|32|64));
}
...
# 900 "builtin.c"
 ...
 r = tmp_string(t1->sub.val.sp + indx, length);
 do { if ((t1)->flags&2) { unref(t1); }} while (0);
 return r;
}
...
NODE *tree;
{
 ...
 (void) time(&lclock);
 return mk_number(((double) lclock), (unsigned int)(1|2|32|64));
}
...
# 1040 "builtin.c"
 ...
 if (cmd && *cmd) {
# 1060 "builtin.c"
  save = cmd[tmp->sub.val.slen];
  cmd[tmp->sub.val.slen] = '\0';

  ret = system(cmd);
  ret = (ret >> 8) & 0xff;

  cmd[tmp->sub.val.slen] = save;
 }
 do { if ((tmp)->flags&2) { unref(tmp); }} while (0);
 return mk_number(((double) ret), (unsigned int)(1|2|32|64));
}
...
# 1114 "builtin.c"
  ...
  if (t[i]->flags & 64) {
   if (OFMTidx == CONVFMTidx)
    (void) ({NODE *_ts = (t[i]); ((_ts->flags & 16) && (_ts->sub.val.idx == -1 || _ts->sub.val.idx == CONVFMTidx)) ? _ts : r_force_string(_ts);});
   else
 ...
 for (i = 0; i < numnodes; i++) {
  efwrite(t[i]->sub.val.sp, sizeof(char), t[i]->sub.val.slen, fp, "print", rp, 0);
  unref(t[i]);
  if (i != numnodes - 1) {
   if (OFSlen > 0)
 ...
 if (ORSlen > 0)
  efwrite(ORS, sizeof(char), (size_t) ORSlen, fp, "print", rp, 1);
 free(t);
}
...
# 1190 "builtin.c"
 ...
 do { if ((t1)->flags&2) { unref(t1); }} while (0);
 do { if ((t2)->flags&2) { unref(t2); }} while (0);
 return mk_number(((double) atan2(d1, d2)), (unsigned int)(1|2|32|64));
}
...
{
 if (firstrand) {
  (void) gawk_initstate((unsigned) 1, state, sizeof state);
  gawk_srandom(1);
  firstrand = 0;
 }
 return mk_number(((double) gawk_random() / 0x7fffffffL), (unsigned int)(1|2|32|64));
}
...
# 1284 "builtin.c"
 ...
 RSTART_node->sub.nodep.l.lptr = mk_number(((double) rstart), (unsigned int)(1|32|64));
 unref(RLENGTH_node->sub.nodep.l.lptr);
 RLENGTH_node->sub.nodep.l.lptr = mk_number((rlength), (unsigned int)(1|32|64));
 return mk_number(((double) rstart), (unsigned int)(1|2|32|64));
}
# 1368 "builtin.c"
static NODE *
...
# 1408 "builtin.c"
 ...
 if (research(rp, t->sub.val.sp, 0, t->sub.val.slen, 1) == -1 ||
     (rp)->regs.start[0] > t->sub.val.slen) {
...
# 1418 "builtin.c"
 ...
 if (t->sub.val.sref > 1 || (t->flags & (4|2048)) != 0) {
...
# 1522 "builtin.c"
 ...
 empty:
  ...
  if ((current >= how_many && !global)
      ...
      || research(rp, t->sub.val.sp, text - t->sub.val.sp, textlen, 1) == -1)
   break;
 ...
 *bp = '\0';
 ...
 free(t->sub.val.sp);
 t->sub.val.sp = buf;
 t->sub.val.slen = textlen;

 do { if ((s)->flags&2) { unref(s); }} while (0);
 if (matches > 0 && lhs) {
  if (priv) {
   unref(*lhs);
   *lhs = t;
...
# 1602 "builtin.c"
 ...
 return mk_number(((double) matches), (unsigned int)(1|2|32|64));
}
...
# 1649 "builtin.c"
 ...
 target = make_str_node((tmp->sub.val.sp), (size_t) (tmp->sub.val.slen), 0);
 do { if ((tmp)->flags&2) { unref(tmp); }} while (0);

 n3 = *(n2.sub.nodep.r.rptr->sub.nodep.r.rptr);
 n3.sub.nodep.l.lptr = target;
 n2.sub.nodep.r.rptr = & n3;

 if ((t->flags & (16|8)) != 0) {
  if (t->sub.val.slen > 0 && (t->sub.val.sp[0] == 'g' || t->sub.val.sp[0] == 'G'))
   how_many = -1;
  else
   how_many = 1;
 } else {
  d = ({NODE *_tn = (t); (_tn->flags & 32) ?_tn->sub.val.fltnum : r_force_number(_tn);});
  if (d > 0)
   how_many = d;
  else
   how_many = 1;
 ...
 ret = sub_common(&n1, how_many, 1);
 do { if ((ret)->flags&2) { unref(ret); }} while (0);
