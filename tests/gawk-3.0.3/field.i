...
static long re_parse_field (long, char **, int, NODE *, Regexp *, Setfunc, NODE *)
                                   ;
static long def_parse_field (long, char **, int, NODE *, Regexp *, Setfunc, NODE *)
                                    ;
static long posix_def_parse_field (long, char **, int, NODE *, Regexp *, Setfunc, NODE *)
                                    ;
static long null_parse_field (long, char **, int, NODE *, Regexp *, Setfunc, NODE *)
                                   ;
static long sc_parse_field (long, char **, int, NODE *, Regexp *, Setfunc, NODE *)
                                   ;
static long fw_parse_field (long, char **, int, NODE *, Regexp *, Setfunc, NODE *)
                                   ;
...
static long parse_high_water = 0;
static long nf_high_water = 0;
 ...
 *n = *Nnull_string;
 ...
 if (nextfree) Null_field = nextfree, nextfree = nextfree->sub.nodep.l.lptr; else Null_field = more_nodes();
 *Null_field = *Nnull_string;
 Null_field->flags |= (512|2048);
 Null_field->flags &= ~(32|64|128|4);
 field0_valid = 1;
}
...
rebuild_record()
{
 ...
 for (i = 1; i <= NF; i++) {
  ...
  if (tmp->sub.val.slen == 1)
   *cops++ = tmp->sub.val.sp[0];
  else if (tmp->sub.val.slen != 0) {
   memcpy(cops, tmp->sub.val.sp, tmp->sub.val.slen);
   cops += tmp->sub.val.slen;
  }

  if (i != NF) {
   if (ofslen == 1)
    *cops++ = ofs->sub.val.sp[0];
   else if (ofslen != 0) {
    memcpy(cops, ofs->sub.val.sp, ofslen);
    cops += ofslen;
   }
  }
 }
 tmp = make_str_node(ops, tlen, 2);
 ...
 f0start = fields_arr[0]->sub.val.sp;
 f0end = fields_arr[0]->sub.val.sp + fields_arr[0]->sub.val.slen;
 for (cops = ops, i = 1; i <= NF; i++) {
  char *field_data = fields_arr[i]->sub.val.sp;

  if (fields_arr[i]->sub.val.slen > 0
      && f0start <= field_data && field_data < f0end)
   fields_arr[i]->sub.val.sp = cops;

  cops += fields_arr[i]->sub.val.slen + ofslen;
 }
...
int freeold;
{
 ...
 if (freeold) {
  ...
  n->type = Node_val;
  n->sub.val.idx = -1;
  n->flags = (8|16|128|512|2048);
  fields_arr[0] = n;
 }
 fields_arr[0]->flags |= 128;
 field0_valid = 1;
}
...
reset_record()
{
 (void) ({NODE *_ts = (fields_arr[0]); ((_ts->flags & 16) && (_ts->sub.val.idx == -1 || _ts->sub.val.idx == CONVFMTidx)) ? _ts : r_force_string(_ts);});
 set_record(fields_arr[0]->sub.val.sp, fields_arr[0]->sub.val.slen, 0);
}
...
NODE *n;
{
 ...
 while (scan < end
  ...
  (*set)(++nf, field,
         (long)(scan + (rp)->regs.start[0] - field), n);
  scan += (rp)->regs.end[0];
  field = scan;
  if (scan == end)
   (*set)(++nf, field, 0L, n);
...
NODE *n;
{
 ...
 if (RS_is_null && fs->sub.val.slen == 0)
  fschar = '\n';
 else
...
NODE *n;
{
 ...
 it = make_str_node((s), (size_t) (len), 0);
 it->flags |= 128;
 *assoc_lookup(n, mk_number(((double) (num)), (unsigned int)(1|2|32|64))) = it;
}
...
NODE *tree;
{
 ...
 if (arr->type == Node_param_list)
  arr = stack_ptr[arr->sub.nodep.number];
 if (arr->type != Node_var && arr->type != Node_var_array)
  set_loc("field.c", 750), r_fatal("second argument of split is not an array");
 arr->type = Node_var_array;
 assoc_clear(arr);

 if (sep->sub.nodep.reflags & 4) {
  parseit = parse_field;
...
set_FS()
{
 ...
 if (! do_traditional && fs->sub.val.slen == 0)
 ...
 else if (fs->sub.val.slen > 1)
  parse_field = re_parse_field;
 else if (RS_is_null) {
  parse_field = sc_parse_field;
  if (fs->sub.val.slen == 1) {
   if (fs->sub.val.sp[0] == ' ') {
    default_FS = 1;
    strcpy(buf, "[ \t\n]+");
   } else if (fs->sub.val.sp[0] != '\n')
    sprintf(buf, "[%c\n]", fs->sub.val.sp[0]);
  }
 } else {
  if (do_posix)
  ...
  if (fs->sub.val.sp[0] == ' ' && fs->sub.val.slen == 1)
   default_FS = 1;
  else if (fs->sub.val.sp[0] != ' ' && fs->sub.val.slen == 1) {
   if (! IGNORECASE)
    parse_field = sc_parse_field;
   else if (fs->sub.val.sp[0] == '\\')
