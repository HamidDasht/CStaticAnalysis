...
const char *filespec;
{
 ...
 p = strrchr(filespec, '/');
 return (p == ((void *)0) ? (char *) filespec : p + 1);
}
...
# 80 "./posix/gawkmisc.c"
 if (isatty(fd))
  return 8192;
 if (fstat(fd, stb) == -1)
  set_loc("./posix/gawkmisc.c", 83), r_fatal("can't stat fd %d (%s)", fd, strerror((*__errno_location ())));
 if (lseek(fd, (off_t)0, 0) == -1)
  return (stb->st_blksize ? stb->st_blksize : 8192);
 if (stb->st_size > 0 && stb->st_size < (stb->st_blksize ? stb->st_blksize : 8192))
  return stb->st_size;
 return (stb->st_blksize ? stb->st_blksize : 8192);
}
...
size_t bytes;
{
 ...
 (void)((p=(pointer)malloc((size_t)(bytes))) || (set_loc("./gawkmisc.c", 60), r_fatal("%s: %s: can't allocate memory (%s)", ("xmalloc"), "p", strerror((*__errno_location ()))),0));
