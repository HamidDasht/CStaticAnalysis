...
void catchsig (int sig, int code) __attribute__((crest_skip));
...
# 176 "main.c"
 ...
 Nnull_string->sub.val.fltnum = 0.0;
 Nnull_string->type = Node_val;
 Nnull_string->flags = (4|16|8|32|64);
...
char *str;
{
 ...
 tmp = ((FS_node)->type == Node_var ? (&(FS_node)->sub.nodep.l.lptr) : r_get_lhs((FS_node), ((Func_ptr *) 0)));
 unref(*tmp);
 ...
 *tmp = make_str_node(str, strlen(str), 1);
 set_FS();
}
 ...
 *aptr = make_str_node((argv0), (size_t) (strlen(argv0)), 0);
 ...
 for (i = argc0, j = 1; i < argc; i++) {
  aptr = assoc_lookup(ARGV_node, mk_number(((double) j), (unsigned int)(1|2|32|64)));
  *aptr = make_str_node((argv[i]), (size_t) (strlen(argv[i])), 0);
  (*aptr)->flags |= 128;
  j++;
 }
...
# 622 "main.c"
  ...
  *aptr = make_str_node((val), (size_t) (strlen(val)), 0);
  (*aptr)->flags |= (128|512);
...
# 667 "main.c"
  ...
  if (badvar)
   set_loc("main.c", 672), r_fatal("illegal name `%s' in variable assignment", arg);





  it = make_str_node(cp, strlen(cp), 1);
  it->flags |= (128|512);
  var = variable(arg, 0, Node_var);
...
# 720 "main.c"
  ...
  r_fatal("internal error line %d, file: %s", 726, "main.c");;
